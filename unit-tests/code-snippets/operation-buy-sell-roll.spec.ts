...
it('should create new operations: buy and sell roll', async () => {
  ...
  const buyOperationId = await web3client
    .wallet()
    .buyRolls({ fee: 0n, amount: 1n });
  const sellOperationId = await web3client
    .wallet()
    .sellRolls({ fee: 0n, amount: 1n });
  ...
  console.log(
    'Buy roll operation id',
    buyOperationId,
    'Sell roll operation id',
    sellOperationId,
  );
});
